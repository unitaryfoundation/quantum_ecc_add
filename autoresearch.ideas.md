...
- **Scale-absorption Kaliski probe**: raw Kaliski emits `-v^-1*2^iters`; pre-scaling the denominator by `2^iters` makes the exposed inverse exact and deletes the pair1 halving / pair2 doubling correction loops. Fast generic prescale was classically right but phase-unsafe (`KAL_PRESCALE_PAIR1=1` saw `altseed_phase_batches_total=1`). Phase-clean exact prescale is wired as `KAL_PRESCALE_PAIR1_SAFE=1` and passes full harness but is much worse: `avg_toffoli=4,786,373`, `qubits=2,972`, clean. Conclusion: algebraic lever real, current prescaler dead; only revisit with a secp256k1-specific phase-clean shifted-add prescaler for sparse `2^iters mod p` (e.g. `2^407=2^151(2^32+977)`) below ~50k compute+uncompute and ideally folded into Kaliski `v_w` initialization.
